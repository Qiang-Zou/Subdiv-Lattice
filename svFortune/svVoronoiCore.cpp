...
namespace sv
{
        ...
        : scanLine()
    {
        ...
        sort(sortedDirections.begin(), sortedDirections.end(), [](const std::pair<int, Real3> &a, const std::pair<int, Real3> &b)
    ...
    void SphericalVoronoiCore::step(Real maxDeltaXi)
    {
        ...
        if (siteEventQueue.size() > 0 && (circleEventQueue.size() == 0 || circleEventQueue[0]->theta - siteEventQueue[0].theta > 1e-5))
        {
    ...
    void SphericalVoronoiCore::handleSiteEvent(site_event &event)
    {
        ...
        else
        {
            ...
            for (beach_line::const_iterator itArc = beach.begin(); itArc != beach.end(); ++itArc)
            {
                auto arc = *itArc;
                beach_line::const_iterator itPrevArc = getPrevArcOnBeach(itArc);
                auto prevArc = *itPrevArc;
                beach_line::const_iterator itNextArc = getNextArcOnBeach(itArc);
                ...
                double phi_start = arc->cell->point.phi - M_PI;
                ...
                double phi_end = arc->cell->point.phi + M_PI;
                if (intNext)
                {
                ...
                {
                    intersectFound = event.phi <= min(phi_start, phi_end) || event.phi >= max(phi_start, phi_end);
                }
                if (intersectFound)
                {
                    if (abs(event.phi - phi_end) < 1e-5)
                    {
                        prevArc = arc;
                        arc = nextArc;
                        itArc = find(beach.begin(), beach.end(), arc);
                        itPrevArc = getPrevArcOnBeach(itArc);
                    ...
                    beach_line::const_iterator itArc2 = beach.insert(itArc, make_shared<beach_arc>(arc->cell));
                    itArc = std::next(itArc2);
                    auto arc2 = *itArc2;
                    beach_line::const_iterator itNewArc = beach.insert(itArc, make_shared<beach_arc>(event.cell));
                    ...
                    std::shared_ptr<vertex> newVertex = std::shared_ptr<vertex>(new vertex(p, event.cell, arc->cell));
                    newVertex->index = vertices.size();
                    newVertex->type = "middleVertex";
                    vertices.push_back(newVertex);

                    arc2->startVertex = newVertex;
                    newArc->startVertex = newVertex;
                    ...
                    auto ce1 = std::make_shared<circle_event>(prevArc, arc2, newArc);
                    if (ce1->theta - event.theta > 1e-5 || abs(ce1->theta - event.theta) < 1e-5)
                    {
                        arc2->circleEvent = ce1;
                        addNewCircleEvent(ce1);
                    }

                    auto ce2 = std::make_shared<circle_event>(newArc, arc, nextArc);
                    if (ce2->theta - event.theta > 1e-5 || abs(ce2->theta - event.theta) < 1e-5)
                    {
                        arc->circleEvent = ce2;
                    ...
                    if (prevArc->circleEvent)
                    {
                        ...
                        auto ce = std::make_shared<circle_event>(*itPrevPrevArc, *itPrevArc, *itArc2);
                        if (ce->theta - event.theta > 1e-5 || abs(ce->theta - event.theta) < 1e-5)
                        {
            ...
            if (!intersectFound)
            {
                std::cout << "has no above arc been found for a new site..." << std::endl;
    ...
    void SphericalVoronoiCore::handleCircleEvent(const circle_event_ptr &event)
    {
        ...
        newVertex->index = vertices.size();
        newVertex->type = "circleCenter";

        arc_i->cell->corners.push_back(newVertex);
        arc_j->cell->corners.push_back(newVertex);
        arc_k->cell->corners.push_back(newVertex);
        ...
        else if (getPrevArcOnBeach(itArcI) == getNextArcOnBeach(itArcK) && isFinished())
        {
            std::cout << "ERROR::>>>the final beachline remain three arcs." << std::endl;
            assert(getPrevArcOnBeach(itArcI) != getNextArcOnBeach(itArcK));
        }
        else
        {
            ...
            if (arc_1->cell->index != arc_i->cell->index && arc_i->cell->index != arc_k->cell->index && arc_1->cell->index != arc_k->cell->index)
            {
                ...
                if (ceI->theta - scanLine.xi > 1e-5 || abs(ceI->theta - scanLine.xi) < 1e-5)
                {
            ...
            if (arc_i->cell->index != arc_k->cell->index && arc_k->cell->index != arc_2->cell->index && arc_i->cell->index != arc_2->cell->index)
            {
                ...
                if (ceK->theta - scanLine.xi > 1e-5 || abs(ceK->theta - scanLine.xi) < 1e-5)
                {
    ...
    SphericalVoronoiCore::beach_line::const_iterator SphericalVoronoiCore::getPrevArcOnBeach(beach_line::const_iterator it) const
    {
        if (it != beach.begin())
        {
            return std::prev(it);
    ...
    SphericalVoronoiCore::beach_line::const_iterator SphericalVoronoiCore::getNextArcOnBeach(beach_line::const_iterator it) const
    {
    ...
    bool SphericalVoronoiCore::isArcOnBeach(const beach_arc_ptr &arc) const
    {
        return find(beach.begin(), beach.end(), arc) != beach.end();
    }

    void SphericalVoronoiCore::addNewSiteEvent(const site_event &event)
    {
        using namespace std;
    ...
    void SphericalVoronoiCore::addNewCircleEvent(const std::shared_ptr<circle_event> &event)
    {
        using namespace std;
        auto it = upper_bound(circleEventQueue.begin(), circleEventQueue.end(), event, compare_circle_event_priority());
        circleEventQueue.insert(it, event);
    }

    void SphericalVoronoiCore::removeCircleEvent(const std::shared_ptr<circle_event> &event)
    {
        using namespace std;
    ...
    void SphericalVoronoiCore::cleanupMiddleVertices()
    {
        ...
        for (auto &v0 : vertices)
        {
            if (v0->cells.size() == 2)
            {
                if (v0->halfEdges.size() != 2)
                    std::cout << "v0->halfedges.size()!=2 " << v0->halfEdges.size() << std::endl;
    ...
    void SphericalVoronoiCore::bindHalfEdgesToCells()
    {
        ...
        for (auto &e : halfEdges)
        {
            ...
            cell_ptr c = nullptr;
            double val = glm::dot(n, c0->point.position);
        ...
        for (auto &c : cells)
        {
            vector<half_edge_ptr> potentialEdges;
            potentialEdges.push_back(c->halfEdges[0]);
            c->halfEdges.erase(c->halfEdges.begin());
            half_edge_ptr current = potentialEdges[potentialEdges.size() - 1];

            while (c->halfEdges.size() > 0)
            {
                bool found = false;
                for (int i = 0; i < c->halfEdges.size(); i++)
                {
                    if (current->end == c->halfEdges[i]->start)
                    {
                        potentialEdges.push_back(c->halfEdges[i]);
                        current->next = potentialEdges[potentialEdges.size() - 1];
                        potentialEdges[potentialEdges.size() - 1]->prev = current;
                        current = potentialEdges[potentialEdges.size() - 1];
                        c->halfEdges.erase(c->halfEdges.begin() + i);
                        found = true;
                        break;
                    }
                    if (current->end == c->halfEdges[i]->end)
                    {
                        vertex_ptr temp = c->halfEdges[i]->start;
                        c->halfEdges[i]->start = c->halfEdges[i]->end;
                        c->halfEdges[i]->end = temp;
                ...
                if (found == false)
                {
                    std::cout << "the cell is not a circle" << std::endl;
                    assert(found);
                }
            }

            potentialEdges[potentialEdges.size() - 1]->next = potentialEdges[0];
            potentialEdges[0]->prev = potentialEdges[potentialEdges.size() - 1];
    ...
    bool SphericalVoronoiCore::intersectWithNextArc(beach_line::const_iterator itArc, Real xi, Point &oPoint) const
    {
    ...
    bool SphericalVoronoiCore::intersectWithPrevArc(beach_line::const_iterator itArc, Real xi, Point &oPoint) const
    {
